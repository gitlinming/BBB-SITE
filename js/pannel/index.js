...
echarts.registerTheme('ve-chart', {
  ...
  // },
  tooltip: {
    extraCssText: 'box-shadow: 0 5px 20px rgba(0, 0, 0, 0.2);',
...
var color = ['#009688', '#FFC107', '#2196F3', '#ffb980', '#0067a6', '#c4b4e4', '#d87a80', '#9cbbff', '#d9d0c7', '#87a997', '#d49ea2', '#5b4947', '#7ba3a8'];
var colorLight = ['#00BCD4', '#FFEB3B', '#03A9F4', '#ffb980', '#0067a6', '#c4b4e4', '#d87a80', '#9cbbff', '#d9d0c7', '#87a997', '#d49ea2', '#5b4947', '#7ba3a8'];
...
var bar = function bar(columns, rows, settings, extra) {
  ...
  var legend = legendVisible && getLegend({ metrics: metrics, labelMap: labelMap });
  ...
  var series = getBarSeries({ innerRows: innerRows, metrics: metrics, stack: stack, axisSite: axisSite, isHistogram: isHistogram, labelMap: labelMap });
...
function getLineSeries(args) {
  ...
  metrics.forEach(function (item,index) {
    ...
    if (areaStyle){
      ...
      //   y2: 1,
      //   colorStops: [
      //     {offset: 1, color: colorLight[index]},
      //     {offset: .2, color: color[index]}
      //   ],
...
function getLineTooltip(args) {
  ...
  return {
    ...
    formatter: function formatter(items) {
      var tpl = [];
      tpl.push('<p style="color: #999;text-align: center;margin-bottom: 6px;">'+items[0].name + '</p>');
      items.forEach(function (item) {
        var showData = null;
...
function getTooltip(dataType, digit, dataStore, metrics, color$$1) {
  return {
    formatter: function formatter(item) {
      var tpl = [];
      tpl.push('<p style="color: #999;text-align: center;margin-bottom: 6px;">'+item.name + '</p>');
...
var VeMap = {
  render: function render() {
    var _vm = this;var _h = _vm.$createElement;var _c = _vm._self._c || _h;return _c('div', { ref: "canvas", staticClass: "ve-map", style: _vm.canvasStyle });
  },
  staticRenderFns: [],
  name: 'VeMap',
  mixins: [chartMixin],
...
var bmap = function bmap(_, __, settings) {
  ...
  return getBmap(key).then(function (_) {
    return { bmap: bmap };
  });
};
...
var VeBmap = {
  render: function render() {
    var _vm = this;var _h = _vm.$createElement;var _c = _vm._self._c || _h;return _c('div', { ref: "canvas", staticClass: "ve-bmap", style: _vm.canvasStyle });
  },
  staticRenderFns: [],
  name: 'VeBmap',
  mixins: [chartMixin],
...
var components = [VeBar, VeHistogram, VeLine, VePie, VeRing, VeWaterfall, VeFunnel, VeRadar, VeChart, VeMap, VeBmap];
